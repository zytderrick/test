...
%%
function f_control_evaluate(vehiclestate)
load("common.mat");
%% 
...
if length(driving_station_error)<2 || length(driving_speed_error)<2 || length(driving_lateral_error)<2 || length(driving_heading_error)<2
    disp("f_control_evaluate:not enough driving data, not calculate");
    disp("return");
    return
end
%% data print
...
datatable = table(index_list(1:end-1),max_value_list(1:end-1),TP999_list(1:end-1),'VariableNames',{'info','max_error','TP999'});
disp(datatable);
end